...
function installSkillsDir() {
  ...
  // Copy the committed hooks/install-mcp-deps.py (canonical source, committed by
  // feat/native-plugin). Using the committed file rather than emitting a duplicate
  // inline string keeps the MCP bootstrap mechanism in one place (dedup).
  const installDepsSrc = path.join(src, 'hooks', 'install-mcp-deps.py');
  const installerPath = path.join(hooksDest, 'install-mcp-deps.py');
  fs.copyFileSync(installDepsSrc, installerPath);
  fs.chmodSync(installerPath, '755');
  console.log(`  ${green}✓${reset} Copied hooks/install-mcp-deps.py`);
